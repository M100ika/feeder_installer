...
BUFFER_SIZE = 1024
RFID_TIMEOUT = int(cfg.get_setting("RFID_Reader", "reader_timeout"))

def __connect_rfid_reader_feeder():
    try:    
        print('Start connect RFID function')
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    print(f'After end: Animal ID: {animal_id}')
                    return animal_id if animal_id != None else None
                except socket.timeout:
                    print(f'Timeout occurred on attempt {attempt}')
        return None
    except Exception as e:
        print(f'Error connect RFID reader {e}')
        return None


def main():
    try:
        print(f'Start test rfid reader antenna\n')
        cow_id = '435400040001'
        while(1):
            cow_id = __connect_rfid_reader_feeder()
            if cow_id != '435400040001':
                print(f'Cow_id now is: {cow_id}\n')
